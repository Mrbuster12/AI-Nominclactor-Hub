(function(){
  ...
  function onCTC(e){
  try{
    var __p = arguments && arguments[0];
    var __k = (typeof __p === "string") ? __p : JSON.stringify(__p||{});
    window.__CTC_SEEN = window.__CTC_SEEN || new Set();
    if (window.__CTC_SEEN.has(__k)) { return; }
    window.__CTC_SEEN.add(__k);
    setTimeout(function(){ window.__CTC_SEEN.delete(__k); }, 800);
  }catch(e){}
